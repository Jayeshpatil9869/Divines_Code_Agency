...
const metrics = [
  { value: 40, suffix: "+", label: "Products shipped", detail: "From MVPs to Series B platforms" },
  { value: 61, suffix: "%", label: "Avg. conversion lift", detail: "Measured 90 days post-launch" },
  { value: 6, suffix: "mo", label: "Avg. engagement", detail: "Embedded, not drive-by" },
  { value: 4, suffix: "", label: "Clients per year", detail: "Depth over volume" },
];
    ...
    <section ref={rootRef} className="w-full py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {metrics.map((m) => (
            <div key={m.label} data-gsap="metric-card" className="border-t border-border pt-6">
              ...
              <div className="text-[11px] uppercase tracking-[0.2em] font-bold mb-2">{m.label}</div>
              <div className="text-[11px] text-muted-foreground font-light">{m.detail}</div>
            </div>
          ))}
        </div>
      </div>
